...
public class FileListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = mInflater.inflate(R.layout.filelist_row, parent,
					false);
		...
		String filename = viewTag.getmFileNode().mName.substring(viewTag
				.getmFileNode().mName.lastIndexOf("/") + 1);
		...
		if (viewTag.getmFileNode().mFormat == Format.mov ||
				...
				viewTag.getmThumbnail().setImageResource(R.drawable.video_type); // for temporary, it should be type_video
			} else {
				viewTag.getmThumbnail().setImageResource(R.drawable.photo_type); // for temporary, it should be type_photo
			}
